<?php
	//POST messages from yellow_pages.php
		$var_value = $_POST['restaurant'];
	...
	// location of the search API endpoint
		$endpoint = "http://api2.yp.com/listings/v1/search?searchloc=".$zip_code."&term=".$term."&format=json&sort=distance&radius=5&listingcount=2";
  ...
  //If Statement. Displays a sorry message if the 'zip code' isn't there (aka listingCount of 0)
    if ($listing >= 1) {
		...
		//First Pizza Place
			echo "The closest".$name."to ".$town_name." is <a href=".$response_pizza_array1['searchResult']['searchListings']['searchListing'][0]['businessNameURL']." target='_blank'> ".$response_pizza_array1['searchResult']['searchListings']['searchListing'][0]['businessName'].".</a><br>";
		...
		//Second Pizza place
			...
			if (isset($pbn2a)) {
				echo "The 2nd closest".$name."to ".$town_name." is <a href=".$response_pizza_array1['searchResult']['searchListings']['searchListing'][1]['businessNameURL']." target='blank'>".$pbn2a.".</a><br>";
